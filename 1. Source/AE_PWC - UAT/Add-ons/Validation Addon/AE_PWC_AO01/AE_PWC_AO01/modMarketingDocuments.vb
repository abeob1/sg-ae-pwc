    ...
    Public Function POValidation_MatrixGridCode(ByRef oForm As SAPbouiCOM.Form, ByVal dDocAmount As Double, ByRef sErrDesc As String) As Long
        ...
        Try
            ...
            If FsPreApproved = "Y" Then
                sSQL = "SELECT T0.[U_ApprGridCode] FROM [dbo].[@AB_APPROVALMATRIX]  T0 WHERE T0.U_ApprDept = '" & sSlpName & "'  and T0.[U_DocType]  = 'PO' and  isnull(T0.[U_PreApprFROM],0)  <= " & dDocAmount & " and  isnull(T0.[U_PreApprTO],0) >= " & dDocAmount & ""
            Else
                If FsBudgeted = "Y" Then
                    sSQL = "SELECT T0.[U_ApprGridCode] FROM [dbo].[@AB_APPROVALMATRIX]  T0 WHERE T0.U_ApprDept = '" & sSlpName & "' and T0.[U_DocType]  = 'PO' and  isnull(T0.[U_BudFROM],0)  <= " & dDocAmount & " and  isnull(T0.[U_BudTO],0)  >= " & dDocAmount & ""
                ElseIf FsBudgeted = "N" Then
                    sSQL = "SELECT T0.[U_ApprGridCode] FROM [dbo].[@AB_APPROVALMATRIX]  T0 WHERE T0.U_ApprDept = '" & sSlpName & "' and T0.[U_DocType]  = 'PO' and  isnull(T0.[U_UnbudFROM],0)  <= " & dDocAmount & " and  isnull(T0.[U_UnbudTO],0)  >= " & dDocAmount & ""
                End If
            End If
...
ST01:
                ...
                If FsPreApproved = "Y" Then
                    sSQL = "SELECT T0.[U_ApprGridCode] FROM [dbo].[@AB_APPROVALMATRIX]  T0 WHERE T0.U_ApprDept = '" & sSlpName & "'  and T0.[U_DocType]  = 'PO' and  isnull(T0.[U_PreApprFROM],0)  <= " & dcomAmount & " and  isnull(T0.[U_PreApprTO],0) >= " & dcomAmount & ""
                Else
                    If FsBudgeted = "Y" Then
                        sSQL = "SELECT T0.[U_ApprGridCode] FROM [dbo].[@AB_APPROVALMATRIX]  T0 WHERE T0.U_ApprDept = '" & sSlpName & "' and T0.[U_DocType]  = 'PO' and  isnull(T0.[U_BudFROM],0)  <= " & dcomAmount & " and  isnull(T0.[U_BudTO],0)  >= " & dcomAmount & ""
                    ElseIf FsBudgeted = "N" Then
                        sSQL = "SELECT T0.[U_ApprGridCode] FROM [dbo].[@AB_APPROVALMATRIX]  T0 WHERE T0.U_ApprDept = '" & sSlpName & "' and T0.[U_DocType]  = 'PO' and  isnull(T0.[U_UnbudFROM],0)  <= " & dcomAmount & " and  isnull(T0.[U_UnbudTO],0)  >= " & dcomAmount & ""
                    End If
                End If
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Getting Approval Matrix Grid Code from [@AB_APPROVALMATRIX] " & sSQL, sFuncName)
                oRset.DoQuery(sSQL)
    ...
    Public Function Budget_Validation(ByRef oNewForm As SAPbouiCOM.Form, ByRef sErrDesc As String) As Long
