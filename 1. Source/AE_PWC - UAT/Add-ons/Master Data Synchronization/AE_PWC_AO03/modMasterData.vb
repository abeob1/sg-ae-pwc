    ...
    Module modMasterData
            ...
            Try
                ...
                If oBP_Holding.GetByKey(sMasterdatacode) Then
                    ...
                    If oBP_Target.GetByKey(sMasterdatacode) Then
                        ...
                        '' oBP_Target = oTragetCompany.GetBusinessObjectFromXML(sFileName, 0)
                        ''/*   Deleting the BP bank in the Target Entity
                        If oBP_Target.BPBankAccounts.Count > 0 Then
                            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Count " & oBP_Target.BPBankAccounts.Count, sFuncName)
                            For imjs As Integer = oBP_Target.BPBankAccounts.Count - 1 To 0 Step -1
                                If String.IsNullOrEmpty(oBP_Target.BPBankAccounts.AccountNo) Then Exit For
                                oBP_Target.BPBankAccounts.SetCurrentLine(imjs)
                                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Delete " & imjs, sFuncName)
                                oBP_Target.BPBankAccounts.Delete()
                                bDelete = True
                                If oBP_Target.BPBankAccounts.Count = 0 Then
                                    Exit For
                                End If
                        ...
                        If bDelete = True Then
                            Dim va As Integer = oBP_Target.Update()
                            If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Delete the House bank value " & va, sFuncName)
                            bDelete = False
                        End If

                        If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Calling BP_Assignment() " & sMasterdatacode, sFuncName)

                        BP_Assignment(oBP_Holding, oBP_Target, sMasterdatacode, sCurrentMsg, True)
        ...
        Public Sub BP_Assignment(ByRef oBPMaster As SAPbobsCOM.BusinessPartners, ByRef oBPMaster_Target As SAPbobsCOM.BusinessPartners, ByVal sSynccode As String, _
            ...
            Try
                ...
                End If
                oBPMaster_Target.SalesPersonCode = oBPMaster.SalesPersonCode
        ...
        Public Sub BP_Assignment_Update(ByRef oBPMaster As SAPbobsCOM.BusinessPartners, ByRef oBPMaster_Target As SAPbobsCOM.BusinessPartners, ByVal sSynccode As String, ByRef sCurrency As String)
