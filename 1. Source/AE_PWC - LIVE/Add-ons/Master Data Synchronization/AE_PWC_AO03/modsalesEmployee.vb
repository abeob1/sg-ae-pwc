...
Namespace AE_PWC_AO03
    ...
    Module modsalesEmployee
        ...
        Public Function SalesEmployeeSync(ByRef oHoldingCompany As SAPbobsCOM.Company, ByRef oTragetCompany As SAPbobsCOM.Company, ByVal sMasterdatacode As String, ByVal sSPLName As String, _
                           ByVal sOSLP As String, ByRef sErrDesc As String) As Long
            ...
            Try
                ...
                Dim oRset As SAPbobsCOM.Recordset = oHoldingCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                Dim oRset_Target As SAPbobsCOM.Recordset = oTragetCompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset)
                Dim oCmpSrv As SAPbobsCOM.CompanyService = oTragetCompany.GetCompanyService
                Dim oDepartmentServicesT As SAPbobsCOM.DepartmentsService = oCmpSrv.GetBusinessService(SAPbobsCOM.ServiceTypes.DepartmentsService)
                Dim oDepartmentT As SAPbobsCOM.Department = oDepartmentServicesT.GetDataInterface(SAPbobsCOM.DepartmentsServiceDataInterfaces.dsDepartment)

                Dim oCmpSrvH As SAPbobsCOM.CompanyService = oHoldingCompany.GetCompanyService
                Dim oDepartmentServicesH As SAPbobsCOM.DepartmentsService = oCmpSrvH.GetBusinessService(SAPbobsCOM.ServiceTypes.DepartmentsService)
                Dim oDepartmentH As SAPbobsCOM.Department = oDepartmentServicesH.GetDataInterface(SAPbobsCOM.DepartmentsServiceDataInterfaces.dsDepartment)

                sSQL = "SELECT T0.[SlpCode], T0.[SlpName] FROM OSLP T0 WHERE T0.[Active] = 'Y' and  T0.[SlpName] = '" & sSPLName & "'" ' order by T0.[SlpCode]"
                If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Listing Sales Employee Query " & sSQL, sFuncName)
                oRset_Target.DoQuery(sSQL)
                Dim sSpLCode_Target As String = String.Empty
                sSpLCode_Target = oRset_Target.Fields.Item("SlpCode").Value
                ...
                If sOSLP = "S" Then
                    ...
                    If oSalesEmployee_Holding.GetByKey(sSalesEmployee) Then
                        Department(oDepartmentH, oDepartmentServicesH, oSalesEmployee_Holding)
                        If oSalesEmployee_Target.GetByKey(sSpLCode_Target) Then
                ...
                ElseIf sOSLP = "D" Then
                    ...
                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug("Listing Sales Employee Query " & sSQL, sFuncName)
                    oRset.DoQuery(sSQL)
                    Dim sSpLCode_Holding As String = String.Empty
                    sSpLCode_Holding = oRset.Fields.Item("SlpCode").Value

                    sSQL = "SELECT T0.[Name], T0.[Remarks], T0.[Father] FROM OUDP T0 WHERE T0.[Name] = '" & sSPLName & "'"
                    If p_iDebugMode = DEBUG_ON Then Call WriteToLogFile_Debug(" Department SQL ", sFuncName)
                    oRset.DoQuery(sSQL)
                    If oRset.RecordCount > 0 Then

                        If oSalesEmployee_Holding.GetByKey(sSpLCode_Holding) Then
                            SalesEmployee(oSalesEmployee_Holding, oRset.Fields.Item("Name").Value, oRset.Fields.Item("Remarks").Value)
                            ival = oSalesEmployee_Holding.Update()
                            If ival <> 0 Then
                                IsError = True
                                oTragetCompany.GetLastError(iErr, sErr)
                                Call WriteToLogFile("Completed with ERROR ---" & sErr, sFuncName)
                        ...
                        Else
                            SalesEmployee(oSalesEmployee_Holding, oRset.Fields.Item("Name").Value, oRset.Fields.Item("Remarks").Value)
                            ival = oSalesEmployee_Holding.Add()
                            If ival <> 0 Then
                                IsError = True
                                oTragetCompany.GetLastError(iErr, sErr)
                                Call WriteToLogFile("Completed with ERROR ---" & sErr, sFuncName)
                        ...
                        If oSalesEmployee_Target.GetByKey(sSpLCode_Target) Then
                            ...
                            SalesEmployee(oSalesEmployee_Target, oRset.Fields.Item("Name").Value, oRset.Fields.Item("Remarks").Value)
                            ival = oSalesEmployee_Target.Update()
                            If ival <> 0 Then
                                IsError = True
                                oTragetCompany.GetLastError(iErr, sErr)
